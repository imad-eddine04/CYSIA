import string
import collections

# Fonction pour effectuer un XOR entre deux chaînes d'octets
def strxor(a, b):
    return bytes([x ^ y for (x, y) in zip(a, b)])

# Les 10 textes chiffrés inconnus (en format hexadécimal), tous chiffrés avec la même clé
c1 = ""
c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
c4 = ""
c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
c6 = ""
c7 = ""
c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
c9 = ""
c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"

# Le texte cible que nous voulons décrypter
target_cipher = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"

# Pour stocker la clé finale
final_key = [None]*150
# Pour stocker les positions que nous savons être cassées
known_key_positions = set()

# Pour chaque texte chiffré
for current_index, ciphertext in enumerate([c1, c2, c3, c4, c5, c6, c7, c8, c9, c10]):

    counter = collections.Counter()
    # Pour chaque autre texte chiffré
    for index, ciphertext2 in enumerate([c1, c2, c3, c4, c5, c6, c7, c8, c9, c10]):
        if current_index != index:  # ne pas xor un texte chiffré avec lui-même
            for indexOfChar, char in enumerate(strxor(bytes.fromhex(ciphertext), bytes.fromhex(ciphertext2))):  # Xor des deux textes chiffrés
                # Xor des deux textes chiffrés
                # Si un caractère dans le résultat xored est un caractère alphanumérique, cela signifie qu'il y avait probablement un espace dans l'un des textes en clair (nous ne savons pas lequel)
                if chr(char) in string.printable and chr(char).isalpha():
                    counter[indexOfChar] += 1  # Incrémenter le compteur à cet index

    knownSpaceIndexes = []

    # Parcourir toutes les positions où un caractère d'espace était possible dans le texte courant
    for ind, val in counter.items():
        # Si un espace a été trouvé au moins 7 fois à cet index sur les 9 XOR possibles, alors le caractère d'espace provenait probablement du texte chiffré courant !
        if val >= 7:
            knownSpaceIndexes.append(ind)

    # Xor le texte courant avec des espaces, et aux positions de knownSpaceIndexes nous obtenons la clé !
    xor_with_spaces = strxor(bytes.fromhex(ciphertext), b' ' * 150)
    for index in knownSpaceIndexes:
        # Stocker la valeur de la clé à la position correcte
        final_key[index] = format(xor_with_spaces[index], '02x')
        # Enregistrer que nous connaissons la clé à cette position
        known_key_positions.add(index)

# Construire une clé hexadécimale à partir de la clé actuellement connue, en ajoutant des caractères hexadécimaux '00' là où nous ne savons pas (pour former une chaîne hexadécimale complète)
final_key_hex = ''.join([val if val is not None else '00' for val in final_key])
# Xor de la clé actuellement connue avec le texte chiffré cible
output = strxor(bytes.fromhex(target_cipher), bytes.fromhex(final_key_hex))
# Afficher la sortie, en imprimant un * si ce caractère n'est pas encore connu
print(''.join([chr(char) if index in known_key_positions else '*' for index, char in enumerate(output)]))
